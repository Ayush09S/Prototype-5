...
public class GameManagerX : MonoBehaviour
{
    ...
    public TextMeshProUGUI gameBeatenText;
    ...
    IEnumerator TimeKeeper()
    {
        while (isGameActive)
        {
            ...
            if (timeCount == 0 && score > 0)
            {
                GameFinished(gameBeatenText);
            }
            if (timeCount == 0 && score <= 0)
            {
                GameFinished(gameOverText);
            }
        }
    }

    // Stop game, bring up game over text and restart button
    public void GameFinished(TextMeshProUGUI gameEnd)
    {
